                                       ...
                                       qDebug()<<"generate1 char 12\n"<<readtemp[11];
                                                    ...
                                                    if(readtemp[9] == 0)
                                                    {
                                                        ...
                                                        if(readtemp[9] == 0)
                                                        {
                                                          ...
                                                          QMessageBox::information(this, tr("Message"),tr("Registration sucess remove your finger  "),QMessageBox::Ok);
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    QTime Ttwale(12,0,0);
    QTime Tone(13,0,0);
    Ttwo(14,0,0),Tthree(15,0,0),Tfoure(16,0,0);

    QString fname("data.csv");
    QString ften("10am.txt");
    QString felven("11am.txt");
    QString ftwale("12pm.txt");
    QString fone("1pm.txt");
    QString ftwo("2pm.txt");
    QString fthree("3pm.txt");
    QString ffour("4pm.txt");

  QFile  file(fname);
  QFile ten(ften);
  QFile elven(felven);
  QFile twale(ftwale);
  QFile one(fone),two(ftwo),three(fthree),four(ffour);

    if(file.open(QIODevice::Append) && ten.open(QIODevice::Append) && elven.open(QIODevice::Append) && twale.open(QIODevice::Append) && one.open(QIODevice::Append) && two.open(QIODevice::Append) && three.open(QIODevice::Append) && four.open(QIODevice::Append))
    {

     QTextStream    out1(&file) , qten(&ten) , qelven(&elven) , qtwale(&twale) , qone(&one),qtwo(&two),qthree(&three),qfour(&four);
    ...
    qDebug()<<intime;
            ...
            else if( 1) // at login time checking
            {
            ...
            if(readtemp[11] == 2)
            {
               ...
               if(intime <= Tten)
               {
                   qten << "log in at" << dateTimeString << "id " <<list[0] << "\n";
               }
                else if(intime <= Telven)
               {
                   qelven << "log in at" << dateTimeString << "id " <<list[0] << "\n";

               }
               else if(intime <= Ttwale)
              {
                  qtwale << "log in at" << dateTimeString << "id " <<list[0] << "\n";

              }
               else if(intime <= Tone)
              {
                  qone << "log in at" << dateTimeString << "id " <<list[0] << "\n";

              }
               else if(intime <= Ttwo)
              {
                  qtwo << "log in at" << dateTimeString << "id " <<list[0] << "\n";

              }
               else if(intime <= Tthree)
              {
                  qthree << "log in at" << dateTimeString << "id " <<list[0] << "\n";

              }
               else if(intime <= Tfoure)
              {
                  qfour << "log in at" << dateTimeString << "id " <<list[0] << "\n";
            ...
            else if(readtemp[11]==3)
            {
                ...
                if(intime <= Tten)
                {
                    qten << "log in at" << dateTimeString << "id " <<list[1] << "\n";
                }
                 else if(intime <= Telven)
                {
                    qelven << "log in at" << dateTimeString << "id " <<list[1] << "\n";

                }
                else if(intime <= Ttwale)
               {
                   qtwale << "log in at" << dateTimeString << "id " <<list[1] << "\n";

               }
                else if(intime <= Tone)
               {
                   qone << "log in at" << dateTimeString << "id " <<list[1] << "\n";

               }
                else if(intime <= Ttwo)
               {
                   qtwo << "log in at" << dateTimeString << "id " <<list[1] << "\n";

               }
                else if(intime <= Tthree)
               {
                   qthree << "log in at" << dateTimeString << "id " <<list[1] << "\n";

               }
                else if(intime <= Tfoure)
               {
                   qfour << "log in at" << dateTimeString << "id " <<list[1] << "\n";
            ...
            else if(readtemp[11] == 4)
            {
                ...
                if(intime <= Tten)
                {
                    qten << "log in at" << dateTimeString << "id " <<list[2] << "\n";
                }
                 else if(intime <= Telven)
                {
                    qelven << "log in at" << dateTimeString << "id " <<list[2] << "\n";

                }
                else if(intime <= Ttwale)
               {
                   qtwale << "log in at" << dateTimeString << "id " <<list[2] << "\n";

               }
                else if(intime <= Tone)
               {
                   qone << "log in at" << dateTimeString << "id " <<list[2] << "\n";

               }
                else if(intime <= Ttwo)
               {
                   qtwo << "log in at" << dateTimeString << "id " <<list[2] << "\n";

               }
                else if(intime <= Tthree)
               {
                   qthree << "log in at" << dateTimeString << "id " <<list[2] << "\n";

               }
                else if(intime <= Tfoure)
               {
                   qfour << "log in at" << dateTimeString << "id " <<list[2] << "\n";
            ...
            else if(readtemp[11] == 5)
            {
                ...
                if(intime <= Tten)
                {
                    qten << "log in at" << dateTimeString << "id " <<list[3] << "\n";
                }
                 else if(intime <= Telven)
                {
                    qelven << "log in at" << dateTimeString << "id " <<list[3] << "\n";

                }
                else if(intime <= Ttwale)
               {
                   qtwale << "log in at" << dateTimeString << "id " <<list[3] << "\n";

               }
                else if(intime <= Tone)
               {
                   qone << "log in at" << dateTimeString << "id " <<list[3] << "\n";

               }
                else if(intime <= Ttwo)
               {
                   qtwo << "log in at" << dateTimeString << "id " <<list[3] << "\n";

               }
                else if(intime <= Tthree)
               {
                   qthree << "log in at" << dateTimeString << "id " <<list[3] << "\n";

               }
                else if(intime <= Tfoure)
               {
                   qfour << "log in at" << dateTimeString << "id " <<list[3] << "\n";
            ...
            else if(readtemp[11] == 6)
            {
                ...
                if(intime <= Tten)
                {
                    qten << "log in at" << dateTimeString << "id " <<list[4] << "\n";
                }
                 else if(intime <= Telven)
                {
                    qelven << "log in at" << dateTimeString << "id " <<list[4] << "\n";

                }
                else if(intime <= Ttwale)
               {
                   qtwale << "log in at" << dateTimeString << "id " <<list[4] << "\n";

               }
                else if(intime <= Tone)
               {
                   qone << "log in at" << dateTimeString << "id " <<list[4] << "\n";

               }
                else if(intime <= Ttwo)
               {
                   qtwo << "log in at" << dateTimeString << "id " <<list[4] << "\n";

               }
                else if(intime <= Tthree)
               {
                   qthree << "log in at" << dateTimeString << "id " <<list[4] << "\n";

               }
                else if(intime <= Tfoure)
               {
                   qfour << "log in at" << dateTimeString << "id " <<list[4] << "\n";
...
{
    ui->status_2->setText("You have no permission \nto go");
}

}
else
    ui->status_2->setText("Place you finger and \n try agin");
...
void MainWindow::on_pushButton_3_clicked()   //checking for reg login success
{
    QString in = "username",out = "password";

    QString in1,out1;
    in1= ui->lineEdit_2->text();
    out1 = ui->lineEdit_3->text();
    if(in == in1 && out == out1)
    {
